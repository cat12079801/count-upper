...
ランニングコスト最小化を優先し、無料枠で完結する構成を提案する。
...
| 認証 / DB / 同期 | Supabase (Auth + Postgres + RLS) | 無料枠で認証・DB・同期を一括提供 |
| ホスティング | Vercel (Hobby) or Cloudflare Pages | 無料枠 |

> 上記は提案。確定後にスキャフォールドする。
